...
static int send_command (mms_io_t *io, mms_t *instance, int command, uint32_t prefix1, uint32_t prefix2, int length) {
	int len8;
	off_t n;
	...
	mms_buffer_init(&command_buffer, instance->scmd);
	...
	mms_buffer_put_32 (&command_buffer, instance->seq_num);
	instance->seq_num++;
	...
	if (length & 7)
		memset(instance->scmd + length + CMD_HEADER_LEN + CMD_PREFIX_LEN, 0, 8 - (length & 7));

	n = io_write(io,	instance->s, instance->scmd, len8 * 8 + CMD_HEADER_LEN + CMD_PREFIX_LEN);
	if (n != (len8 * 8 + CMD_HEADER_LEN + CMD_PREFIX_LEN)) {
		return 0;
	}

	print_command (instance->scmd, length);
...
* return packet type
*/
static int get_packet_header (mms_io_t *io, mms_t *instance, mms_packet_header_t *header) {
	size_t len = 0;
	int packet_type;
	...
	len = io_read(io, instance->s, instance->buf, 8);
	instance->buf_packet_seq_offset = -1;
	if (len != 8)
		goto error;

	if (LE_32(instance->buf + 4) == 0xb00bface) {
		/* command packet */
		header->flags = instance->buf[3];
		len = io_read(io,	instance->s, instance->buf + 8, 4);
		if (len != 4)
			goto error;

		header->packet_len = LE_32(instance->buf + 8) + 4;
		...
		header->packet_seq			= LE_32(instance->buf);
		header->packet_id_type		= instance->buf[4];
		header->flags				= instance->buf[5];
		header->packet_len			= (LE_16(instance->buf + 6) - 8) & 0xffff;
...
static int get_packet_command (mms_io_t *io, mms_t *instance, uint32_t packet_len) {
	...
	len = io_read(io,	instance->s, instance->buf + 12, packet_len) ;
	//instance->buf_packet_seq_offset = -1; // already set in get_packet_header
	if (len != packet_len) {
		return 0;
	}

	print_command (instance->buf, len);

	/* check protocol type ("MMS ") */
	if (LE_32(instance->buf + 12) != 0x20534D4D) {
		return 0;
	}

	command = LE_32 (instance->buf + 36) & 0xFFFF;

	return command;
}

static int get_answer (mms_io_t *io, mms_t *instance) {
	int command = 0;
	mms_packet_header_t header;

	switch (get_packet_header (io, instance, &header)) {
		case MMS_PACKET_ERR:
			// failed to read mms packet header
			break;
		case MMS_PACKET_COMMAND:
			command = get_packet_command (io, instance, header.packet_len);

			if (command == 0x1b) {

				if (!send_command (io, instance, 0x1b, 0, 0, 0)) {
					// failed to send command
					return 0;
				}
				/* FIXME: limit recursion */
				command = get_answer (io, instance);
			}
			break;
		case MMS_PACKET_ASF_HEADER:
			// unexpected asf header packet
			break;
		case MMS_PACKET_ASF_PACKET:
			// unexpected asf packet
			break;
	}
...
static int get_asf_header (mms_io_t *io, mms_t *instance) {
	...
	while (!stop && !g_bStopMms) {
		mms_packet_header_t header;
		int command;

		switch (get_packet_header (io, instance, &header)) {
			case MMS_PACKET_ERR:
				// failed to read mms packet heade
				return 0;
				break;
			case MMS_PACKET_COMMAND:
				command = get_packet_command (io, instance, header.packet_len);
			...
			case MMS_PACKET_ASF_HEADER:
			case MMS_PACKET_ASF_PACKET:
				if (header.packet_len + instance->asf_header_len > ASF_HEADER_LEN) {
					//					asf packet too large
					return 0;
				}
				len = io_read(io,	instance->s,
					instance->asf_header + instance->asf_header_len, header.packet_len);
				if (len != header.packet_len) {
					/* FIXME: de-xine-ification */
					//				get_asf_header failed
					return 0;
				}
				instance->asf_header_len += header.packet_len;
...
static void interp_asf_header (mms_t *instance) {

	int i, pass;

	instance->asf_packet_len = 0;
	...
	*/
	...
	for(pass = 0; pass < 2; ++pass) {
		i = 30;
		while (i < (int) instance->asf_header_len) {
			...
			switch (guid) {
				...
				case GUID_ASF_FILE_PROPERTIES:
					if(pass) {
						instance->asf_packet_len = LE_32(instance->asf_header + i + 92 - 24);
						if (instance->asf_packet_len > BUF_SIZE) {
							instance->asf_packet_len = 0;
							// asf packet len too large
							break;
						}
						instance->file_len			= LE_64(instance->asf_header + i + 40 - 24);
				...
				case GUID_ASF_STREAM_PROPERTIES:
					if(!pass) {
						...
						if (instance->num_stream_ids < ASF_MAX_NUM_STREAMS) {
							instance->streams[instance->num_stream_ids].stream_type = type;
							instance->streams[instance->num_stream_ids].stream_id = stream_id;
				...
				case GUID_ASF_STREAM_BITRATE_PROPERTIES:
					if(pass) {
						...
						for(j = 0; j < streams; j++) {
							...
							for(stream_index = 0; stream_index < instance->num_stream_ids; stream_index++) {
								if (instance->streams[stream_index].stream_id == stream_id)
									break;
							}
							if (stream_index < instance->num_stream_ids) {
								instance->streams[stream_index].bitrate = LE_32(instance->asf_header + i + 4 + j * 6);
								instance->streams[stream_index].bitrate_pos = i + 4 + j * 6;
							}
						}
					}
					break;
...
* returns 1 on error
*/
static int mms_tcp_connect(mms_io_t *io, mms_t *instance) {
	int progress, res;

	if (!instance->port)
		instance->port = MMST_PORT;

	/* 
	* try to connect 
	*/
	instance->s = io_connect(io, instance->host, instance->port);
	if (instance->s == -1) {
		// failed to connect
		return 1;
	}
	...
	do {
		...
		progress += 1;
	} while ((res == MMS_IO_STATUS_TIMEOUT) && (progress < 15) && (!g_bStopMms));
...
* return 0 on error
*/
int static mms_choose_best_streams(mms_io_t *io, mms_t *instance) {
	int i;
	int video_stream = 0;
	int audio_stream = 0;
	int max_arate = 0;
	...
	for (i = 0; i < instance->num_stream_ids; i++) {
		switch (instance->streams[i].stream_type) {
			case ASF_STREAM_TYPE_AUDIO:
				if ((int) instance->streams[i].bitrate > max_arate) {
					audio_stream = instance->streams[i].stream_id;
					max_arate = instance->streams[i].bitrate;
				}
				break;
			default:
				break;
	...
	bandwitdh_left = instance->bandwidth - max_arate;
	if (bandwitdh_left < 0) {
		bandwitdh_left = 0;
	}
	...
	for (i = 0; i < instance->num_stream_ids; i++) {
		switch (instance->streams[i].stream_type) {
			case ASF_STREAM_TYPE_VIDEO:
				if (((bandwitdh_left - (int) instance->streams[i].bitrate) < min_bw_left) &&
					(bandwitdh_left >= (int) instance->streams[i].bitrate)) {
						video_stream = instance->streams[i].stream_id;
						min_bw_left = bandwitdh_left - instance->streams[i].bitrate;
				}
				break;
			default:
				break;
	...
	if (!video_stream && instance->has_video) {
		for (i = 0; i < instance->num_stream_ids; i++) {
			switch (instance->streams[i].stream_type) {
				case ASF_STREAM_TYPE_VIDEO:
					if (((int) instance->streams[i].bitrate < min_vrate) ||
						(!min_vrate)) {
							video_stream = instance->streams[i].stream_id;
							min_vrate = instance->streams[i].bitrate;
					}
					break;
				default:
					break;
	...
	memset (instance->scmd_body, 0, 40);
	for (i = 1; i < instance->num_stream_ids; i++) {
		instance->scmd_body [ (i - 1) * 6 + 2 ] = 0xFF;
		instance->scmd_body [ (i - 1) * 6 + 3 ] = 0xFF;
		instance->scmd_body [ (i - 1) * 6 + 4 ] = instance->streams[i].stream_id ;
		instance->scmd_body [ (i - 1) * 6 + 5 ] = instance->streams[i].stream_id >> 8;
		if ((instance->streams[i].stream_id == audio_stream) ||
			(instance->streams[i].stream_id == video_stream)) {
				instance->scmd_body [ (i - 1) * 6 + 6 ] = 0x00;
				instance->scmd_body [ (i - 1) * 6 + 7 ] = 0x00;
		} else {
			instance->scmd_body [ (i - 1) * 6 + 6 ] = 0x02;
			instance->scmd_body [ (i - 1) * 6 + 7 ] = 0x00;

			/* forces the asf demuxer to not choose this stream */
			if (instance->streams[i].bitrate_pos) {
				instance->asf_header[instance->streams[i].bitrate_pos] = 0;
				instance->asf_header[instance->streams[i].bitrate_pos + 1] = 0;
				instance->asf_header[instance->streams[i].bitrate_pos + 2] = 0;
				instance->asf_header[instance->streams[i].bitrate_pos + 3] = 0;
			}
		}
	}

	if (!send_command (io, instance, 0x33, instance->num_stream_ids, 
		0xFFFF | instance->streams[0].stream_id << 16, 
		instance->num_stream_ids * 6 + 2)) {
			// mms_choose_best_streams failed
			return 0;
	}

	if ((res = get_answer (io, instance)) != 0x21) {
		// unexpected response
	}
...
mms_t* mms_connect (mms_io_t *io, void *data, const char *url, int bandwidth) {
	...
	instance = (mms_t*) calloc(1, sizeof(mms_t));
	...
	instance->s = -1;
	instance->scmd_body = instance->scmd + CMD_HEADER_LEN + CMD_PREFIX_LEN;
	instance->need_discont = 1;
	instance->buf_packet_seq_offset = -1;
	instance->bandwidth = bandwidth;
	if(uri->scheme)
		instance->proto = strdup(uri->scheme);
	if(uri->user)
		instance->user = strdup(uri->user);
	if(uri->passwd)
		instance->password = strdup(uri->passwd);
	if(uri->hostname)
		instance->host = strdup(uri->hostname);
	...
	if (!mmst_valid_proto(instance->proto)) {
		// unsupported protocol
		goto fail;
	}

	if (mms_tcp_connect(io, instance)) {
		goto fail;
	}
	...
	*/
	...
	/* command 0x1 */
	mms_gen_guid(instance->guid);
	sprintf(instance->str, "\x1c\x03NSPlayer/7.0.0.1956; {%s}; Host: %s",
		instance->guid, instance->host);
	string_utf16(url_conv, instance->scmd_body, instance->str, strlen(instance->str) + 2);

	if (!send_command(io, instance, 1, 0, 0x0004000b, strlen(instance->str) * 2 + 8)) {
		// failed to send command 0x01
		goto fail;
	}

	if ((res = get_answer(io, instance)) != 0x01) {
		// unexpected response
		goto fail;
	}
	...
	/* command 0x2 */
	string_utf16(url_conv, &instance->scmd_body[8], "\002\000\\\\192.168.0.129\\TCP\\1037\0000", 28);
	memset (instance->scmd_body, 0, 8);
	if(!send_command (io, instance, 2, 0, 0, 28 * 2 + 8)) {
		// failed to send command 0x02
		goto fail;
	}

	switch (res = get_answer (io, instance)) {
		case 0x02:
			// protocol accepted
			break;
		case 0x03:
	...
	/* command 0x5 */
	{
		...
		mms_buffer_init(&command_buffer, instance->scmd_body);
		...
		if (strlen(instance->uri) >= CMD_BODY_LEN - 64)
			goto fail;

		string_utf16(url_conv, instance->scmd_body + command_buffer.pos, instance->uri, strlen(instance->uri));
		if (!send_command (io, instance, 5, 1, 0xffffffff, strlen(instance->uri) * 2 + 12))
			goto fail;
	}
	...
	case 0x06:
		{
			...
			xx = instance->buf[62];
			yy = instance->buf[63];
			instance->live_flag = ((xx == 0) && ((yy & 0xf) == 2));
			instance->seekable = !instance->live_flag;
		}
		break;
	case 0x1A:
	...
	/* command 0x15 */
	{
		...
		mms_buffer_put_32(&command_buffer, 0x00000000);					/* ?? */
		if (!send_command (io, instance, 0x15, 1, 0, command_buffer.pos)) {
			// failed to send command 0x15
			goto fail;
		}
	}

	if ((res = get_answer (io, instance)) != 0x11) {
		// unexpected response
		goto fail;
	}

	if (!get_asf_header (io, instance))
		goto fail;

	interp_asf_header (instance);

	if (!instance->asf_packet_len || !instance->num_stream_ids)
		goto fail;

	if (!mms_choose_best_streams(io, instance)) {
		// mms_choose_best_streams failed
		goto fail;
	}

	/* command 0x07 */
	instance->packet_id_type = ASF_MEDIA_PACKET_ID_TYPE;
	{
		mms_buffer_t command_buffer;
		mms_buffer_init(&command_buffer, instance->scmd_body);
		...
		mms_buffer_put_32(&command_buffer, instance->packet_id_type);	/* asf media packet id type */
		if (!send_command(io, instance, 0x07, 1, 0x0001FFFF, command_buffer.pos)) {
			goto fail;
		}
	}
...
static int get_media_packet (mms_io_t *io, mms_t *instance) {
	mms_packet_header_t header;
	off_t len;
		...
		case 0x1e:
			{
				...
				error_code = LE_32(instance->buf + CMD_HEADER_LEN);
		...
		case MMS_PACKET_ASF_PACKET:
			{
				...
				if(instance->need_discont &&
					header.packet_id_type == ASF_MEDIA_PACKET_ID_TYPE)
				{
					instance->need_discont = 0;
					instance->start_packet_seq = header.packet_seq;
				}

				if (header.packet_len > instance->asf_packet_len) {
					// invalid asf packet len
					return 0;
				}

				/* simulate a seek */
				instance->current_pos = (off_t)instance->asf_header_len +
					((off_t)header.packet_seq - instance->start_packet_seq) * (off_t)instance->asf_packet_len;

				len = io_read(io,	instance->s, instance->buf, header.packet_len);
				if (len != header.packet_len) {
					/* FIXME: de-xine-ification */
					return 0;
				}
				...
				/* explicit padding with 0 */
				{
					...
					char *end	= start + instance->asf_packet_len - header.packet_len;
					if ((start > base) && (start < (base+BUF_SIZE-1)) &&
						(start < end)	&& (end < (base+BUF_SIZE-1))) {
							memset(instance->buf + header.packet_len, 0, instance->asf_packet_len - header.packet_len);
					}
					if (header.packet_id_type == instance->packet_id_type) {
						if (instance->asf_packet_len > BUF_SIZE) {
							instance->buf_size = BUF_SIZE;
						} else {
							instance->buf_size = instance->asf_packet_len;
						}
						instance->buf_packet_seq_offset =
							header.packet_seq - instance->start_packet_seq;
					} else {
						instance->buf_size = 0;
						// Don't set instance packet sequence for reuse in seek(), since the
						// subsequence packet may be discontinued.
						//instance->buf_packet_seq_offset = header.packet_seq;
...
int mms_read (mms_io_t *io, mms_t *instance, char *data, int len) {
	...
	total = 0;
	while (total < len && !instance->eos && !g_bStopMms) {
		...
		if (instance->asf_header_read < instance->asf_header_len) {
			...
			if (bytes_left == 0) {
				...
				if (!get_media_packet (io, instance)) {
					// get_media_packet failed
					return total;
				}
...
static int mms_request_data_packet (mms_io_t *io, mms_t *instance, double time_sec, mms_off_t first_packet, unsigned long time_msec_limit) {
	/* command 0x07 */
	{
		mms_buffer_t command_buffer;
		//mms_buffer_init(&command_buffer, instance->scmd_body);
		//mms_buffer_put_32 (&command_buffer, 0x00000000);									/* 64 byte float timestamp */
		//mms_buffer_put_32 (&command_buffer, 0x00000000);									
		memcpy(instance->scmd_body, &time_sec, 8);
		mms_buffer_init(&command_buffer, instance->scmd_body+8);
		mms_buffer_put_32 (&command_buffer, 0xFFFFFFFF);									/* ?? */
		//mms_buffer_put_32 (&command_buffer, 0xFFFFFFFF);									/* first packet sequence */
		mms_buffer_put_32 (&command_buffer, (uint32_t) first_packet);									/* first packet sequence */
		//mms_buffer_put_8	(&command_buffer, 0xFF);												/* max stream time limit (3 bytes) */
		//mms_buffer_put_8	(&command_buffer, 0xFF);
		//mms_buffer_put_8	(&command_buffer, 0xFF);
		//mms_buffer_put_8	(&command_buffer, 0x00);												/* stream time limit flag */
		...
		mms_buffer_put_32 (&command_buffer, instance->packet_id_type);		/* asf media packet id type */
		if (!send_command (io, instance, 0x07, 1, 0x0001FFFF, 8+command_buffer.pos)) {
			// ailed to send command 0x07
			return 0;
		}
	}
...
int mms_request_time_seek (mms_io_t *io, mms_t *instance, double time_sec) {
	if (++instance->packet_id_type <= ASF_MEDIA_PACKET_ID_TYPE)
		instance->packet_id_type = ASF_MEDIA_PACKET_ID_TYPE+1;
	//return mms_request_data_packet (io, instance, time_sec, 0xFFFFFFFF, 0x00FFFFFF);
	// also adjust time by preroll
	return mms_request_data_packet (io, instance,
		time_sec+(double)(instance->preroll)/1000,
...
// the first packet.
// To be inline function?
static int peek_and_set_pos (mms_io_t *io, mms_t *instance) {
	uint8_t saved_buf[BUF_SIZE];
	int		saved_buf_size;
	off_t	saved_buf_packet_seq_offset;
	// save buf and buf_size that may be changed in get_media_packet()
	memcpy(saved_buf, instance->buf, instance->buf_size);
	saved_buf_size = instance->buf_size;
	saved_buf_packet_seq_offset = instance->buf_packet_seq_offset;
	//instance->buf_size = instance->buf_read = 0; // reset buf, only if success peeking
	instance->buf_size = 0;
	while (!instance->eos && !g_bStopMms) {
		...
		if (!get_media_packet (io, instance)) {
			...
			memcpy(instance->buf, saved_buf, saved_buf_size);
			instance->buf_size = saved_buf_size;
			instance->buf_packet_seq_offset = saved_buf_packet_seq_offset;
			return 0;
		}
		if (instance->buf_size > 0) break;
	}
...
int mms_time_seek (mms_io_t *io, mms_t *instance, double time_sec) {
	...
	if (!mms_request_time_seek (io, instance, time_sec)) return 0;
	return peek_and_set_pos (io, instance);
}

// http://sdp.ppona.com/zipfiles/MMSprotocol_pdf.zip said that, instance
...
int mms_request_packet_seek (mms_io_t *io, mms_t *instance, mms_off_t packet_seq) {
	if (++instance->packet_id_type <= ASF_MEDIA_PACKET_ID_TYPE)
		instance->packet_id_type = ASF_MEDIA_PACKET_ID_TYPE+1;
	return mms_request_data_packet (io, instance, 0, packet_seq, 0x00FFFFFF);
}
...
static mms_off_t mms_packet_seek (mms_io_t *io, mms_t *instance, mms_off_t packet_seq) {
	if (!mms_request_packet_seek (io, instance, packet_seq))
		return 0;
	return peek_and_set_pos (io, instance);
}

/*
TODO: To use instance table to calculate buf_packet_seq_offset rather than store
and retrieve it from instance->buf_packet_seq_offset?
...
mms_off_t mms_seek (mms_io_t *io, mms_t *instance, mms_off_t offset, int origin) {
	...
	switch (origin) {
		...
		case SEEK_CUR:
			...
			break;
		case SEEK_END:
			//if (instance->asf_num_packets == 0) {
			//	//printf ("input_mms: unknown end position in seek!\n");
			//	return instance->current_pos;
			//}
			dest = mms_get_length (instance) + offset;
		default:
	...
	//if (dest_packet_seq > 0) dest_packet_seq /= instance->asf_packet_len;
	dest_packet_seq = dest_packet_seq >= 0 ?
		dest_packet_seq / instance->asf_packet_len : -1;
#if 0
	// buf_packet_seq_offset will identify which packet sequence are residing in
	// the buf.
#if 1 /* To show both of the alternate styles :D */
	buf_packet_seq_offset = instance->current_pos - instance->asf_header_len;
	//if (buf_packet_seq_offset > 0) buf_packet_seq_offset /= instance->asf_packet_len;
	buf_packet_seq_offset = buf_packet_seq_offset >= 0 ?
		buf_packet_seq_offset / instance->asf_packet_len : -1;
	// Note: buf_read == buf_size == 0 may means that it is eos,
	//			eos means that the packet has been peek'ed.
	if (instance->buf_read >= instance->buf_size && instance->buf_size > 0 &&
		buf_packet_seq_offset >= 0 ||
		// assuming packet not peek'ed in the following condition
		/*instance->buf_read >= instance->buf_size && */instance->buf_size == 0 &&
		buf_packet_seq_offset == 0)
		// The buf is all read but the packet has not been peek'ed.
		--buf_packet_seq_offset;
#else
	buf_packet_seq_offset = instance->current_pos - instance->asf_header_len - 1;
	//if (buf_packet_seq_offset > 0) buf_packet_seq_offset /= instance->asf_packet_len;
	buf_packet_seq_offset = buf_packet_seq_offset >= 0 ?
		buf_packet_seq_offset / instance->asf_packet_len : -1;
	// Note: buf_read == buf_size == 0 may means that it is eos,
	//			eos means that the packet has been peek'ed.
	if (instance->buf_read == 0/* && buf_packet_seq_offset >= 0*/)
	...
	if (dest_packet_seq < 0) {
		if (instance->buf_packet_seq_offset > 0) {
			if (!mms_request_packet_seek (io, instance, 0xFFFFFFFF))
				return instance->current_pos;
#if 1
			// clear buf
			instance->buf_read = instance->buf_size = 0;
			instance->buf_packet_seq_offset = -1;
		} else {
#else
			// clear buf
			instance->buf_read = instance->buf_size;
		...
		} else if (instance->buf_packet_seq_offset == 0) {
#endif
			// reset buf_read
			instance->buf_read = 0;
		}
	...
	if (instance->asf_num_packets && dest == instance->asf_header_len +
		instance->asf_num_packets*instance->asf_packet_len) {
	...
	if (dest_packet_seq != (int64_t) instance->buf_packet_seq_offset) {
		if (instance->asf_num_packets > 0 && dest_packet_seq >= (int64_t) instance->asf_num_packets) {
			// Do not seek beyond the last packet.
			return instance->current_pos;
		}
		if (!mms_packet_seek (io, instance, instance->start_packet_seq + dest_packet_seq))
			return instance->current_pos;
		...
		// reached.
		if (dest_packet_seq != instance->buf_packet_seq_offset)
			return instance->current_pos;
	...
	// eos is reached ?
	//if (instance->buf_size <= 0) return instance->current_pos;
	//instance->buf_read = (dest - instance->asf_header_len) % instance->asf_packet_len;
	instance->buf_read = (int) (dest - (instance->asf_header_len + dest_packet_seq*instance->asf_packet_len));
	// will never happen ?
	//if (instance->buf_size <= instance->buf_read) return instance->current_pos;
	return instance->current_pos = dest;
}
...
uint32_t mms_get_length (mms_t *instance) {
	...
	if (instance->asf_num_packets)
		return instance->asf_header_len + (uint32_t) (instance->asf_num_packets*instance->asf_packet_len);
	else
		return (uint32_t) instance->file_len;
}
...
uint64_t mms_get_asf_packet_len (mms_t *instance) {
	return instance->asf_packet_len;
}
